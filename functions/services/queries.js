...
const queries = {
  ...
  DELETE_SUBSCRIPTIONS: `update subscriptions set deleted_on = now() from event_types where event_types.id = subscriptions.event_type_id and event_types.service_id = $1 and deleted_on = ${zeroTimestamp}`,
  DELETE_EVENTS: `update events set deleted_on = now() from users where users.id = events.user_id and user.service_id = $1 and deleted_on = ${zeroTimestamp} returning id, name`,
  DELETE_SERVICE: `update service set deleted_on = now() where service_id = $1 and deleted_on = ${zeroTimestamp}`,
};
