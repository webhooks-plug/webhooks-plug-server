...
const queries = {
  ...
  GET_EVENT_TYPES: `select e.id, e.name, s.name "serviceName", e.topic_arn from event_types e join services s on s.id = e.service_id where s.id = $1 and s.deleted_on = '${zeroTimestamp}' and e.deleted_on = '${zeroTimestamp}' order by e.created_on desc`,
