...
const queries = {
  ...
  GET_SUBSCRIPTIONS: `select sub.id, et.name as "eventTypeName", en.url as "endpointUrl" from subscriptions sub join endpoints en on en.id = sub.endpoint_id join event_types et on et.id = sub.event_type_id where sub.deleted_on = '${zeroTimestamp}' and et.id = $1`,
